...
class JobRequestService {
    ...
    if (user == null) {
      try {
        ...
        if (savedUserId != null && savedUserId.trim().isNotEmpty) {
          print('🔍 No authenticated user, using persisted skilled worker ID');
          return savedUserId.trim();
        }
      } catch (e) {
        print('❌ Error reading persisted skilled worker ID: $e');
      }
      print('🔍 No authenticated user and no persisted userId');
      return null;
    }
  ...
  static Future<bool> isSkilledWorkerAssignedToJob({
    ...
    try {
      ...
      // Admin panel assignment fallback: check Job/Jobs doc assignedWorkerId
      try {
        final jobDoc = await _firestore.collection('Job').doc(jobId).get();
        final jobData = jobDoc.data();
        final assignedWorkerId =
            (jobData?['assignedWorkerId'] ?? jobData?['workerId'])?.toString();
        if (assignedWorkerId != null && assignedWorkerId == skilledWorkerId) {
          print(
            '✅ Found Job assignment for skilled worker $skilledWorkerId and job $jobId',
          );
          return true;
        }
      } catch (_) {}
      ...
      try {
        ...
        if (assignedWorkerId != null && assignedWorkerId == skilledWorkerId) {
          print(
            '✅ Found Jobs assignment for skilled worker $skilledWorkerId and job $jobId',
          );
          return true;
        }
      } catch (_) {}
